  ...
  Drupal.behaviors.biodex_menu = {
    ...
    var mainMenu = {
      ...
      getBreakpoint: function() {
        let breakpoint = window.getComputedStyle(document.body, null).getPropertyValue("min-width");
      ...
      setMenuHeight: function() {
        ...
        if (this.subMenus.length > 0) {
          const submenusArray = Array.from(this.subMenus);

          submenusArray.forEach(function (submenu) {

            // Gets total height of all non-image items
            let submenuItems = Array.from(submenu.getElementsByClassName('sub-menu-item'));
            let submenuHeight = 0;
            ...
            let submenuImages = Array.from(submenu.getElementsByClassName('sub-menu-image-item'));
      ...
      mobileSubListener: function() {
        ...
        const menuItems = this.topUl.getElementsByClassName('top-sub-menu-item');
       ...
       * @param parentItem
       */
      toggleSubmenu: function(e, parentItem) {


        let subMenu = parentItem.getElementsByClassName('sub-menu')[0];
       ...
       *
       */
      unsetSubheight: function() {

        if(this.getBreakpoint() == 'large') {
          return;
        }

        const subMenus = this.topUl.getElementsByClassName('sub-menu');
      ...
      initMenu: function() {
        ...
        if(settings.positionStickySupport) {
          ...
          if ($(context).find('.product-tabs')) {
            let productTabsOffset          = headerOffset + toolBarOffset;
