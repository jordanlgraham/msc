...
class InsertFileWidgetTest extends InsertFileTestBase {
  ...
  public function testDefaultWidgetElement() {
    ...
    $this->assertEquals(1, count($page->findAll('css', '[name="' . $field_name . '[0][insert_template][link]"]')), 'Insert link template exists');
  ...
  public function testInsertStyleSelectDefault() {
    $field_name = strtolower($this->randomMachineName());
    $this->createFileField($field_name, 'article', [], [], [
      'insert_styles' => ['link' => 'link', 'icon_link' => 'icon_link'],
    ]);
    ...
    $this->assertEquals(1, count($page->findAll('css', '[name="' . $field_name . '[0][insert_template][link]"]')), 'Insert link template exists');
    $this->assertEquals(1, count($page->findAll('css', '[name="' . $field_name . '[0][insert_template][icon_link]"]')), 'Insert icon link template exists');
  ...
  public function testAbsoluteUrlSetting() {
    ...
    $this->createFileField($field_names[1], 'article', [], [], [
      'insert_absolute' => '1',
    ]);
